...
//=============================================================================
 /*:
 * @plugindesc YEP动态敌人及侧视战斗系统[v1.20]
...
dead     死亡
-----------------------------------
*此插件需要YEP_BattleEngineCore。确保此插件位于插件列表中的YEP_BattleEngineCore下。
*此扩展插件允许您以多种方式为敌人设置动画，从赋予静态敌人呼吸、浮动和缩放属性，到利用动画侧视图演员作为敌人的潜在战斗者，而不是静态图形，帮助使敌人看起来更生动！
*如果您正在使用YEP_X_ActSeqPack2，并且希望能够添加浮动敌人，请将此插件也放在YEP_X_ActSeqPack2下。
*要使用此插件，请在敌人的备注框中插入您在以下部分中看到的备注标签：
 * ============================================================================
 * 标签
 * ============================================================================
 *
 ...
 *
 *   <Breathing Speed: x>
 *   一个完整的呼吸周期需要多少帧？x值越低，敌人的呼吸速度就越快。x值越高，敌人呼吸越慢。
 *
 *   <Breathing Rate X: x.y>
 *   <Breathing Rate Y: x.y>
 *  将水平和垂直呼吸频率设置为x.y.1.0是100%的方差变化，而0.0是0%的方差变化。
 *
 *   <Enable HP Link Breathing>
 *   <Disable HP Link Breathing>
 *  将启用/禁用HP链接呼吸。敌人的生命值越低，敌人的呼吸就越慢。
 *
 *   <Floating>  漂浮必须加插件YEP_X_ActSeqPack2.js
 *   将敌人设置为漂浮的动画。
 *
 *   <Floating Speed: x>
 *   一个完整的浮动循环需要多少帧？x值越低，敌人漂浮的速度就越快。x值越高，敌人漂浮的速度就越慢。
 *
 *   <Floating Rate: x.y>
 *   将敌人的浮动速率设置为x.y.1.0是100%的方差变化，而0.0是0%的方差变化。
 *
 *   <Floating Height: x>
 *   将敌人的最小漂浮高度设置为x。
 *
 *   <Floating Death>
 *   <No Floating Death>
 *   决定这个特定的敌人是否会在死亡时漂浮，或者立即倒地，并绕过特定敌人的“漂浮死亡”插件参数。
 *
 *   <Scale Sprite: x%>
 *   这允许您将精灵缩放为原始精灵大小的x%。如果只想缩放宽度或高度，请使用下面的注释标记：
 *
 *   <Scale Sprite Width: x%>
 *   <Scale Sprite Height: x%>
 *   这将专门按x%的量缩放精灵的宽度或高度，而不是按相同的比例缩放整个精灵本身。
 *
 *   --- 侧视图 ---
 *
 *   <Sideview Battler: filename>
 *   这是用于在项目的img/sv_actors/文件夹中找到的侧视图战斗器的文件名。这样做将使以下注释标签能够应用于战斗者。这是区分大小写的，使用时没有图像的文件扩展名。
 *
 *   *素材文件SF_Actor3_8.png 替换：filename   例如：<Sideview Battler: SF_Actor3_8>
 *
 *   *Note: 如果使用这些标签中的一个以上，则将从随机池中选择侧视图战斗者。但是，为了简单起见，它们的设置将与notetag中设置的所有其他侧视图设置相匹配。
 *
 *   --- 侧视图特定 ---
 *
 *   <Sideview Anchor X: y.z>
 *   <Sideview Anchor Y: y.z>
 *   这将敌人侧视图战斗器的锚定位置设置为y.z。这用于当你有一个奇数比例的侧视图战斗机时。
 *
 *   <Sideview Width: x>
 *   <Sideview Height: x>
 *   设置侧视图战斗符的宽度/高度。这是针对你正在使用的战斗者图像的事件，该图像的比例可能与正常的侧视图战斗者不同。
 *
 *   <Sideview Collapse>
 *   设置它，使敌人在死亡时崩溃并消失。
 *
 *   <Sideview No Collapse>
 *   设置它，使敌人在死亡时留下一具尸体，不会消失。
 *
 *   <Sideview Frame Speed: x>
 *   将此侧视图战斗器的帧速设置为x。x值越低，侧视图战斗者的动画制作速度就越快。它越高，战斗者的动画就越慢。
 *
 *   --- 覆盖 ---
 *
 *   <Sideview Show State Overlay>
 *   <Sideview Hide State Overlay>
 *   这将显示或隐藏侧视图敌人的状态覆盖，并忽略插件参数中的默认设置。
 *
 *   --- 动作 ---
 *
 ...
 *   6 - Staff   12 - Spear     18 - Iron Pipe  24 - Stun Rod   30 - Custom
 ----------------------------------------------------------------------------
  *1-匕首   7-长弓   13-梅斯   19-弹弓   25-书籍
  *2-剑     8-十字弓 14-杆     20-霰弹枪 26-自定义
  *3-连发片 9-枪     15-俱乐部 21-步枪   27-自定义
  *4-斧     10-爪    16-链     22-电锯   28-自定义
  *5-鞭子   11-手套  17-剑#2   23-轨道炮 29-自定义
  *6-杖     12-矛    18-铁管   24-特技棒 30-自定义
 *
 *   * 注意：插入多个这样的笔记标签将把它们放在一个随机的武器库中使用。请记住，如果您使用此notetag，它将使用插件参数中的所有默认设置。如果您希望使用更独特的设置，请使用下面的notetag：
 *
 *   <Sideview Weapon: x, y, z>
 *   这将角色的武器图像设置为x，运动设置为y，攻击动画设置为z。如何使用此注释标签的示例
 *   
 *      <Sideview Weapon: 2, swing, 6>
 *
 *   这将给战斗者一把带有摆动动作的剑，并在进攻时播放战斗动画6。
 *
 *   <Sideview Idle Motion: x>
 *   设置侧视图敌人的步行动作。可以使用以下任意运动：
 *   walk     wait     chant     guard     damage     evade
 *   thrust   swing    missile   skill     spell      item
 *   escape   victory  dying     abnormal  sleep      dead
 ...
 *  为你的侧视敌人设置受伤的动作。可以使用以下任意运动：
 *   walk     wait     chant     guard     damage     evade
 *   thrust   swing    missile   skill     spell      item
 *   escape   victory  dying     abnormal  sleep      dead
 *
 *   <Sideview Evade Motion: x>
 *   为你的侧视敌人设置躲避动作。可以使用以下任意运动：
 *   walk     wait     chant     guard     damage     evade
 *   thrust   swing    missile   skill     spell      item
 *   escape   victory  dying     abnormal  sleep      dead
 *
 *   <Sideview Escape Motion: x>
 *   设置侧视图敌人的逃跑动作。可以使用以下任意运动：
 *   walk     wait     chant     guard     damage     evade
 *   thrust   swing    missile   skill     spell      item
 *   escape   victory  dying     abnormal  sleep      dead
 *
 *   <Sideview Guard Motion: x>
 *   设置侧视敌人的防御动作。可以使用以下任意运动：
 *   walk     wait     chant     guard     damage     evade
 *   thrust   swing    missile   skill     spell      item
 *   escape   victory  dying     abnormal  sleep      dead
 *
 *   <Sideview Abnormal Motion: x>
 *   为你的侧视敌人设置异常动作。可以使用以下任意运动：
 *   walk     wait     chant     guard     damage     evade
 *   thrust   swing    missile   skill     spell      item
 *   escape   victory  dying     abnormal  sleep      dead
 *
 *   <Sideview Sleep Motion: x>
 *   为侧视敌人设置睡眠动作。可以使用以下任意运动：
 *   walk     wait     chant     guard     damage     evade
 *   thrust   swing    missile   skill     spell      item
 *   escape   victory  dying     abnormal  sleep      dead
 *
 *   <Sideview Dying Motion: x>
 *  为你的侧视敌人设置濒死（危机）动作。可以使用以下任意运动：
 *   walk     wait     chant     guard     damage     evade
 *   thrust   swing    missile   skill     spell      item
 *   escape   victory  dying     abnormal  sleep      dead
 *
 *   <Sideview Dead Motion: x>
 *   设置侧视图敌人阵亡动作。可以使用以下任意运动：
 *   walk     wait     chant     guard     damage     evade
 *   thrust   swing    missile   skill     spell      item
 *   escape   victory  dying     abnormal  sleep      dead
 *
 ...
 *   将其设置为使敌人为其侧视图精灵显示其阴影。该选项的默认设置与战斗引擎核心的“显示阴影”相关。
 *
 *   <Sideview Hide Shadow>
 *   将其设置为使敌人隐藏其侧视图精灵的阴影。该选项的默认设置与战斗引擎核心的“显示阴影”相关。
 *
 *   <Sideview Shadow Width: x%>
 *   将阴影宽度设置为比img/系统文件夹中的默认阴影大小大/小x%。
 *
 *   <Sideview Shadow Height: x%>
 *   将阴影高度设置为比img/system文件夹中的默认阴影大小大/小x%。
 *
 * 状态备注标签：
 *
 *   <Hide Sideview Weapon>
 *   这将导致动画侧视图敌方战斗者隐藏其侧视图武器效果。攻击动作将恢复为为敌人设置的徒手攻击动作，并且攻击动画将是敌人的默认攻击动画。
 *
 * ============================================================================
 * 更新
 * ============================================================================
 *
 * Version 1.20:
 * - Bugfix provided by YoraeRasante regarding the animation positioning on animated sideview enemies.
 *
 * Version 1.19:
 * - Bugfix provided by SwiftIllusion regarding the animation positioning on animated sideview enemies.
 *
 * Version 1.18:
 ...
 *
 * Version 1.10:
 * - Optimized plugin to use less resources. Animated enemies will no longer have a static graphic once the game is loaded.
 *
 * Version 1.09:
