...
//=============================================================================
 /*:
 * @plugindesc YEP动作序列拓展 3 [v1.03]
 * @author Yanfly Engine Plugins
 *
 * @param Camera Option
 * @text 摄像机选项
 * @desc 用于战斗中显示的、与摄像机移动相关的选项
 * @default 战斗摄像机
 *
 * @help
 * ============================================================================
 ...
 *
《战斗引擎核心》（Battle Engine Core）集成了 Yanfly Engine Melody 的战斗引擎系统，
 在该系统中，技能与物品效果的各个独立环节均可实现一定程度的控制。
 这些可控制的环节被称为 “行动序列”（Action Sequences），序列中的每一条指令都会触发游戏执行一个独立且明确的行动。
 
 每个技能与物品均由五个不同的行动序列构成，具体如下：
 
 准备行动（Setup Actions）
 该序列用于在执行核心行动及各项独立效果前，为当前战斗者（active battler，指当前发起技能 / 物品使用的角色或单位）完成准备工作。
 通常包括当前战斗者小幅前移、拔出武器等动作。此步骤会在当前战斗者消耗技能或物品的 “代价”（如魔法值、技能次数、物品数量等）之前执行。
 
 全体行动（Whole Actions）
  该序列的效果会同时作用于所有目标。尽管此环节并非必需，但大多数行动会通过该序列为所有敌人播放动画（如群体法术的范围特效）。
  此步骤在消耗技能或物品的 “代价” 之后执行。
  
 目标行动（Target Actions）
 该序列的效果会单独作用于每个目标，主要用于物理攻击类行动 —— 这类攻击通常会对单个目标造成更具针对性的伤害。
 除非有明确指令设定，否则此环节中的行动不会影响其他目标。
 
 后续行动（Follow Actions）
 该序列主要用于在 “目标行动” 环节结束后执行收尾工作，例如移除 “不死标记”（immortal flags，
 游戏中使角色暂时不会倒下的状态标识）、触发公共事件（common events，可在多个场景中复用的预设事件）等。
 
 结束行动（Finish Actions）
 该序列用于让当前战斗者完成整个行动序列的收尾。通常包括技能 / 物品效果的最终等待与停滞（如技能命中后的延迟特效）、返回初始位置等动作。
 了解了每个行动序列需经历的五个步骤后，以下是可插入到技能与物品中的标签（tags），请留意每个标签的具体名称。
 *
