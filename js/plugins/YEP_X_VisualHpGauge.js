 ...
 * @param Defeat First
 * @text 击败敌人生命值条显示
 * @desc 只有先攻击敌人后，才会显示其生命值条
 * NO - false     YES - true
 * @default false
 *
 * @param Always Visible
 * @text 设置生命值条的显示状态
 * @desc 生命值条始终保持可见状态，不会自动消失
 * NO - false     YES - true
 * @default false
 *
 * @param ---外观---
 ...
 * @param Y Buffer
 * @text 生命值条 Y 轴
 * @desc 调整生命值条在 Y 轴方向上的偏移量
 * @default -16
 *
 * @param Use Thick Gauges
 * @text 使用厚量规
 * @desc 是否使用本插件提供的粗样式生命值条
 * 默认（false）/ 粗样式（true）
 * @default true
 *
 * @param ---文本显示---
